...
class ChatQAPipeline:
    ...
    def run_with_payload(self, user_input: str) -> Tuple[str, str]:
        ...
        entities, resolution, relationship = self._extract_entities_and_relationships(cleaned_input)
        embedding = self._create_embedding(entities, resolution, relationship)
    ...
    def _extract_entities_and_relationships(self, cleaned_input: str):
        ...
        _ = pipe.extract_relationships()
        df_final = pipe.extract_resolution()
        entities = df_final['entities'].values[0]
        resolution = df_final['resolution'].values[0]
        relationship = df_final['relationship'].values[0]
        return entities, resolution, relationship

    def _create_embedding(self, entities, resolution, relationship):
        fixed_relationships = self.db.fix_relationships(relationship, resolution)
        return self.db.text_to_embedding(entities, fixed_relationships).tolist()
